...
int copypagetable_usertokernel(pagetable_t upagetable, uint begin, uint end, pagetable_t kpagetable)
{
  if ((end < begin) || (PGROUNDUP(end) >= PLIC))
    return -1;
  ...
  for (a = begin; a < end; a += PGSIZE)
  {
    if ((pte = walk(upagetable, a, 0)) == 0)
      panic("copypagetable_usertokernel: pte should exist");
    if ((*pte & PTE_V) == 0)
      panic("copypagetable_usertokernel: page not present");

    if ((kernel_pte = walk(kpagetable, a, 1)) == 0)
      panic("copypagetable_usertokernel: kernel page table walk");
    *kernel_pte = (*pte) & (~PTE_U);
  }
  return 0;
}

pagetable_t proc_kernelpagetable(struct proc *p)
{
  ...
  mappages(kernel_pagetable, TRAPFRAME, PGSIZE, (uint64)(p->trapframe), PTE_R | PTE_W);
  ...
  if (pa == 0)
    return 0;
  uint64 va = KSTACK(0);
  mappages(kernel_pagetable, va, PGSIZE, (uint64)pa, PTE_R | PTE_W);
  p->kstack = va;
...
{
  // 内核页表：内核映射 + 用户映射
  // 该函数中不能释放 内核空间 及 用户空间 对应的物理内存，因此只释放 page table
  // !!!p->kstack 只有内核页表中有映射，用户页表中没有，记得要在该函数前手动将对应的物理内存释放掉
  for (int i = 0; i < 512; i++)
  {
...
void userinit(void)
{
  ...
  p->sz = PGSIZE;
  if (copypagetable_usertokernel(p->pagetable, 0, p->sz, p->kernelpagetable) != 0)
    return;
...
int growproc(int n)
{
  ...
  if (n > 0)
  {
    ...
    if ((sz = uvmalloc(p->pagetable, sz, sz + n)) == 0)
      return -1;
    if (copypagetable_usertokernel(p->pagetable, p->sz, sz, p->kernelpagetable) != 0)
      return -1;
  }
  else if (n < 0)
  {
    ...
    if (PGROUNDUP(sz) < PGROUNDUP(p->sz))
    {
      ...
      uvmunmap(p->kernelpagetable, PGROUNDUP(sz), npages, 0);
    }
  }
  p->sz = sz;
  return 0;
...
int fork(void)
{
  ...
  // lab3-3 start
  if (copypagetable_usertokernel(np->pagetable, 0, np->sz, np->kernelpagetable) != 0)
  {
    freeproc(np);
    release(&np->lock);
    return -1;
